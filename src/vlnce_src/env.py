...
def load_my_datasets(splits):
    ...
    for split in splits:
        ...
        # Load Json
        data_dir = str(Path(args.TRAIN_VOCAB).parent)
        with open('{}/{}.json'.format(data_dir, split), 'r', encoding='utf-8') as f:
...
class AirVLNENV:
    def __init__(self, batch_size=8, split='train',
        ...
        if args.collect_type in ['TF']:
            ...
            if args.run_type in ['collect']:
                ...
                # 续传：跳过已收集的 episodes
                if self.lmdb_features_start_id > 0:
                    resume_idx = (self.lmdb_features_start_id // self.batch_size) * self.batch_size
                    logger.info('Resume from index {} -> {} (skipping {} already-collected episodes)'.format(
                        self.index_data, resume_idx, resume_idx - self.index_data))
                    self.index_data = resume_idx
        ...
        while True:
            try:
                ...
                if (not args.ablate_rgb or not args.ablate_depth):
                    ...
                    if os.environ.get("AIRVLN_ATTACH_EXISTING_SCENE", "0").lower() in ("1", "true", "yes"):
                        self.simulator_tool.attach_existing_scenes()
                    else:
    ...
    def _setEpisodes(self):
        ...
        if (not args.ablate_rgb or not args.ablate_depth):
            ...
            if not result:
                ...
                try:
                    # A persistent Scene16 AirSim instance is already warm.
                    # Avoid paying the fixed two-second startup delay for
                    # every episode, while retaining bounded retries if the
                    # renderer has not produced a frame yet.
                    reuse_scene = os.environ.get(
                        "AIRVLN_ATTACH_EXISTING_SCENE", "0"
                    ).lower() in ("1", "true", "yes")
                    self.simulator_tool.wait_for_image_streams(
                        retry_count=3,
                        retry_sleep=0.5 if reuse_scene else 2.0,
                        initial_sleep=0.0 if reuse_scene else 2.0,
                    )
                except Exception as e:
                    logger.warning(
                        'Post-pose image stream warmup failed, continuing with standard retries: %s',
                        e,
                    )
    ...
    def _update_DistanceToGoal(self):
        for i, state in enumerate(self.sim_states):
            ...
            goal_pos = None
            if 'goals' in state.episode_info and len(state.episode_info['goals']) > 0:
                goal_pos = state.episode_info['goals'][0]['position']
            elif 'reference_path' in state.episode_info and len(state.episode_info['reference_path']) > 0:
                goal_pos = state.episode_info['reference_path'][-1]
