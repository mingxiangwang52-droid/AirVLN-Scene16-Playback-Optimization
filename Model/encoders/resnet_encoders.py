    ...
    def __init__(
        ...
        if checkpoint != "NONE":
            ...
            for k, v in ddppo_weights["state_dict"].items():
                parts = k.split(".")
                if len(parts) < 3 or parts[1] != "visual_encoder":
                    continue
            ...
            def _map_ckpt_key(k):
                ...
                while i < len(parts):
                    ...
                    if p == 'cn':
                        ...
                        new_parts.append('layer' + p[1:])
                    elif p == 'c1':
                        new_parts.extend(('convs', '0'))
                    elif p == 'g1':
                        new_parts.extend(('convs', '1'))
                    elif p == 'c2':
                        new_parts.extend(('convs', '3'))
                    elif p == 'g2':
                        new_parts.extend(('convs', '4'))
                    elif p == 'c3':
                        new_parts.extend(('convs', '6'))
                    elif p == 'g3':
                        new_parts.extend(('convs', '7'))
                    elif p == 'down1':
            ...
            mapped_weights = {_map_ckpt_key(k): v for k, v in weights_dict.items()}
            self.visual_encoder.load_state_dict(mapped_weights, strict=True)
