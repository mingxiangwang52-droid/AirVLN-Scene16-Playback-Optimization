...
class SimState:
    def __init__(self, index=-1,
        ...
        self.NDTW = {
            '_metric': 0,
            'locations': [],
            'gt_locations': (np.array(episode_info.get('reference_path', [[0,0,0]]))[:, 0:3]).tolist(),
        }
        self.SDTW = {
            '_metric': 0,
        }
...
class ENV:
    def __init__(self, load_scenes: list):
        ...
        if args.run_type in ['collect', 'train'] and args.collect_type in ['dagger', 'SF']:
            if args.collect_reference_path_teacher:
                self.shortest_path_sensor = None
                self.nav_graph_token_dict = {}
                logger.warning('Using graph-free reference_path teacher for DAgger collection')
            else:
    ...
    def get_obs_at(self, index: int, state):
        ...
        if args.run_type in ['collect', 'train'] and args.collect_type in ['TF']:
            ...
            if state.is_end:
                teacher_action = AirsimActions.STOP
            elif args.collect_reference_path_teacher:
                teacher_action, state = get_reference_path_teacher_action_at(index, state)
            else:
                teacher_action, state = get_teacher_action_at(index, state, self.nav_graph_token_dict, self.shortest_path_sensor)

            done = state.is_end
            if args.collect_reference_path_teacher:
                progress, state = get_reference_path_progress_at(index, state)
            else:
...
def get_reference_path_teacher_action_at(index: int, state: SimState):
    """Label a visited state by steering it back onto the official reference path."""
    path = state.episode_info['reference_path']
    actions = state.episode_info.get('actions', [])
    position = np.array([
    ...
    # Preserve exact demonstrations while using recovery labels after deviation.
    if state.step < len(path) and state.step < len(actions) and np.allclose(
        position, np.asarray(path[state.step][0:3]), atol=1e-3
    ):
        return actions[state.step], state

    waypoint, state = find_waypoint_at(index, state)
    target = np.asarray(waypoint[0:3], dtype=np.float64)
    delta = target - position
    horizontal_distance = float(np.linalg.norm(delta[0:2]))
    vertical_distance = float(delta[2])

    goal = np.asarray(path[-1][0:3], dtype=np.float64)
    if state.pre_carrot_idx >= len(state.unique_path) - 1 and np.linalg.norm(goal - position) <= 3.0:
        return AirsimActions.STOP, state

    # AirSim uses NED coordinates: a smaller z value means ascending.
    vertical_priority = abs(vertical_distance) >= 3.0
    if vertical_priority or horizontal_distance <= 2.5:
        if vertical_distance <= -1.0:
            return AirsimActions.GO_UP, state
        if vertical_distance >= 1.0:
            return AirsimActions.GO_DOWN, state

    if horizontal_distance > 1e-3:
        yaw = airsim.to_eularian_angles(state.pose.orientation)[2]
        target_yaw = math.atan2(delta[1], delta[0])
        yaw_error = _normalize_angle(target_yaw - yaw)
        if yaw_error > math.radians(AirsimActionSettings.TURN_ANGLE * 0.6):
            return AirsimActions.TURN_RIGHT, state
        if yaw_error < -math.radians(AirsimActionSettings.TURN_ANGLE * 0.6):
            return AirsimActions.TURN_LEFT, state
        return AirsimActions.MOVE_FORWARD, state

    return AirsimActions.STOP, state


def get_reference_path_progress_at(index: int, state: SimState):
    _, state = find_waypoint_at(index, state)
    denominator = max(len(state.unique_path) - 1, 1)
    progress = min(max(float(state.pre_carrot_idx) / denominator, 0.0), 1.0)
