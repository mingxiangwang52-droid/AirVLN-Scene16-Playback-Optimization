...
def _capture_setting(image_type, width, height):
    return {
...
def _preview_camera_settings():
    width = int(os.environ.get("AIRVLN_PREVIEW_CAMERA_WIDTH", "768"))
    height = int(os.environ.get("AIRVLN_PREVIEW_CAMERA_HEIGHT", "432"))
    max_payload_bytes = int(os.environ.get("AIRVLN_PREVIEW_CAMERA_MAX_RAW_BYTES", "1000000"))
    if width * height * 3 > max_payload_bytes:
        scale = (float(max_payload_bytes) / float(width * height * 3)) ** 0.5
        width = max(224, int(width * scale))
        height = max(224, int(height * scale))
    return {
        "CaptureSettings": [
            _capture_setting(0, width, height),
        ],
        "X": 0.5, "Y": 0, "Z": 0,
    ...
    for i in range(drone_num_per_env):
        ...
        drone = {
            ...
            "Cameras": {
                ...
                # Keep front_0 at the model's 224x224 training resolution, but expose
                # a display-only camera so the live demo is not stretched from 224px.
                "preview_0": _preview_camera_settings(),
            },
            "X": 0, "Y": 0, "Z": 0,
            "Pitch": 0, "Roll": 0, "Yaw": 0
        }
    ...
    def _open_scenes(self, ip: str , scen_ids: list):
        ...
        def _check_scene(index, p):
            ...
            if p.poll() is not None:
                print(
                    "{}\tScene launcher exited early for {}-th scene (scene {})\tgpu:{}".format(
                        str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())),
                        index,
                        scen_ids[index],
                        gpus[index],
